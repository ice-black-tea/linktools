...
void usage() {
    printf(
        "usage: adbd [OPTION]\n"
        "\n"
        " -p --port PORT        listen port, default is 5555\n"
        " -d --daemon           run as daemon\n"
        " -D --debug            debug mode\n"
        " --root_seclabel       root_seclabel\n"
...
int main(int argc, char **argv)
{
    ...
    D("Gears adbd\n");
    int is_daemon = 0;
    int port = DEFAULT_ADB_LOCAL_TRANSPORT_PORT; // DEFAULT_ADB_PORT;
    ...
    while(1) {
        ...
        c = getopt_long(argc, argv, "s:b:dp:Dh", opts, &option_index);
        if (c == -1)
            break;
        switch (c) {
        case 's':
        ...
        case 'p':
            ...
            break;
        case 'D':
            adb_trace_mask = 0xFFFFFFFF; // enable all mask
            break;
        case 'h':
        default:
            usage();
    ...
    start_device_log();
    D("Handling main()\n");
    return adb_main(is_daemon, port);
#endif
}
